...
test.describe.serial('Items API', () => {
  ...
  test('GET /api/v1/items?search=Test - should filter items by search', async ({ request }) => {
    const response = await request.get('/api/v1/items?search=Test');

    expect(response.status()).toBe(200);
    const body = await response.json();
    expect(body.data).toBeInstanceOf(Array);
    expect(body.data.every((item: any) =>
      item.title.includes('Test') || item.description?.includes('Test')
    )).toBeTruthy();
  });

  test('GET /api/v1/items/{id} - should get item by ID', async ({ request }) => {
  ...
  test('GET /api/v1/items/999999 - should return 404 for non-existent item', async ({ request }) => {
  ...
  test('GET /api/v1/items/{id}/children - should get item children', async ({ request }) => {
    const response = await request.get(`/api/v1/items/${createdItemId}/children`);
  ...
  test('GET /api/v1/items/{id}/qr - should get QR code for item', async ({ request }) => {
  ...
  test('DELETE /api/v1/items/{id} - should delete item', async ({ request }) => {
  ...
  test('POST /api/v1/items - should validate required fields', async ({ request }) => {
    const response = await request.post('/api/v1/items', {
      data: {
